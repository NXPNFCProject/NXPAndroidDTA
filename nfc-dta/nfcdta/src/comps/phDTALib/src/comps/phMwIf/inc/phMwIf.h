...
typedef uint32_t phMwIf_RfTechMask;
...
typedef union phMwIf_uTagOpsParams
{
...
typedef struct phMwIf_sNciRfIntfIsoDepPollAParams
{
    ...
    BOOLEAN       blNadUsed;                                       /**< NAD is used or not               */
...
{
    uint16_t     wServerHandle;/**< Server Handle for which conn is requested*/
    uint16_t     wConnHandle;  /**< Connection Handle for which conn is requested*/
...
/** \ingroup grp_mwif_lib
    ...
    Actual data can be received by calling  phMwIf_LlcpRecvData*/
...
{
    phMwIf_RfTechMask dwPollRdWrt; /**< Technologies to enable for Read/Write in Poll Mode*/
    phMwIf_RfTechMask dwPollP2P;    /**< Technologies to enable for P2P in Poll Mode*/
    phMwIf_RfTechMask dwListenP2P;  /**< Technologies to enable for P2P in Listen Mode*/
    phMwIf_RfTechMask dwListenHCE;  /**< Technologies to enable for Host Card Emulation in Listen Mode*/
    phMwIf_RfTechMask dwListenESE;  /**< Technologies to enable for Card Emulation from Ese in Listen Mode*/
    phMwIf_RfTechMask dwListenUICC; /**< Technologies to enable for Card Emulation from UICC in Listen Mode*/
 ...
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during init
 * \param[in] discCfgParams       Technology types for poll and listen modes #phDtaLib_DiscMask_t
 *                                CardEmulation Type if enabled.
...
/**
 * \ingroup grp_mwif_lib
 * \brief Intitialize the card emulation for UICC/eSE
 *
 * This function register the card emulation on UICC/eSE
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during CE init
 * \param[in] eDevType            Dev Type specifies whether it is UICC/ESE. HBE is ignored.
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB EE init successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB EE init failed
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_EeInit(void* mwIfHandle,
                                         phMwIf_eCeDevType_t eDevType);

/**
 * \ingroup grp_mwif_lib
 * \brief Configures the controller for card emulation on uicc/eSE
 *
 * This function configures the NFCC to work as card emulation on uicc/eSE
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during EE config
 * \param[in] eDevType            Dev Type specifies whether it is UICC/ESE. HBE is ignored.
 * \param[in] dwRfTech            Specifies Listen A/B/F for technology based routing for specified device
 *                                Poll Types are ignored
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB EE config successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB EE config failed
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_EeConfigure(void* mwIfHandle,
                                              phMwIf_eCeDevType_t eDevType,
                                              phMwIf_RfTechMask   dwRfTechMask);

/**
 * \ingroup grp_mwif_lib
 * \brief Deintitialize the card emulation for UICC/eSE
 *
 * This function register the card emulation on UICC/eSE
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during CE deinit
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB EE de-init successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB EE de-init failed
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_EeDeInit(void* mwIfHandle);

/**
 * \ingroup grp_mwif_lib
 * \brief Intitialize the card emulation over Host
 *
 * This function register the card emulation over host
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during HCE init
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB HCE init successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB HCE init failed
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_HceInit(void* mwIfHandle);
 ...
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during HCE config
 * \param[in] dwRfTech            Specifies Listen A/B for technology based routing for specified device
 *                                Poll Types are ignored
 *
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_HceConfigure(void* mwIfHandle,
                                               phMwIf_RfTechMask   dwRfTechMask);
 ...
 * \param[in] mwIfHandle          Middleware Interface Handle provided during HCE init
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB HCE De-init successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB HCE De-init failed
 *
 */
 ...
 * \param[in] mwIfHandle         Middleware Interface Handle
 *
 * \retval #MWIFSTATUS_SUCCESS  MWIF LIB Configuration is successful
 * \retval #MWIFSTATUS_FAILED   MWIF LIB Failed to configure
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_SetConfig(void* mwIfHandle,
                                    uint8_t uConfigID,
                                    uint8_t ulen,
                                    uint8_t * psBuf);
...
/**
 * \ingroup grp_mwif_lib
 * \brief Set the proprietary configuration for NFC controller before starting the discovery loop.
 *
 * This function Set the proprietary configuration for NFC controller before starting the discovery loop.
 *
 * \param[in] uConfigID          NCI proprietary Configurations ID
 * \param[in] ulen               Length of proprietary configuration ID data/buffer
 * \param[in] psBuf              Configuration ID data buffer
 * \param[in] mwIfHandle         Middleware Interface Handle
 *
 * \retval #MWIFSTATUS_SUCCESS  MWIF LIB proprietary Configuration is successful
 * \retval #MWIFSTATUS_FAILED   MWIF LIB Failed to configure proprietary ID
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_SetConfigProp(void* mwIfHandle,
                                    uint16_t uConfigID,
                                    uint8_t ulen,
                                    uint8_t * psBuf);
 ...
 *
 * This function receives P2P loopback data from middleware with default parameters
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during init
 * \param[in] pvOutBuff           Output Buffer required data handling
 * \param[in] dwLenOutBuff        Length of OutBuffer
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_ReceiveP2PData(void *mwIfHandle,
                                    void* pvOutBuff,
                                    uint32_t* dwLenOutBuff);
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_LlcpRecvData(void*                     pvMwIfHandle,
                                      void*                     pvConnHandle,
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_LlcpSendData(void*                     pvMwIfHandle,
                                               void*                     pvConnHandle,
                                               phMwIf_sBuffParams_t*     psData);
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_LlcpClientConnect( void*                               pvMwIfHandle,
 ...
 * \for client/remote server OR server/remote client
 *
 * This function register LLCP client
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during init
 ...
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_LlcpDisconnectConn( void*     pvMwIfHandle,
                                                      void*     pvConnHandle);
/**
 * \ingroup grp_mwif_lib
 * \brief Connect client to server
