...
#endif
    dwMwStatus |= phMwIfi_ResetDiscoveryConfig(mwIfHandle);
    if(dwMwStatus != MWIFSTATUS_SUCCESS)
    {
                              ...
                              phMwIf_RfTechMask   dwRfTechMask)
{
    ...
    /*NFA_TECHNOLOGY_MASK is aligned with phMwIf_RfTechMask.So assigning it directly*/
    technologiesSwitchOn = (UINT8) dwRfTechMask;
    ...
    if(px_data != NULL)/* Get the Status */
    {
        ...
        switch(uevent)
        {
            ...
            case NFA_DEACTIVATED_EVT:
            {
                ...
                uEvtInfo.eDeactivateType = px_data->deactivated.type;
                                 ...
                                 uint32_t dwLength)
{
    ...
    NDEF_MsgInit((UINT8 *)gs_paramBuffer,(UINT32)600,
              (UINT32 *)&u32psize);
 ...
 */
MWIFSTATUS phMwIf_LlcpDisconnectConn( void*     pvMwIfHandle,
                                        void*     pvConnHandle)
{
    phMwIf_sHandle_t *mwIfHdl = pvMwIfHandle;
    tNFA_STATUS bNfaStatus;
...
void phMwIfi_NfaNxpNciAgcDbgRspCallback(UINT8 event, UINT16 param_len, UINT8 *p_param)
{
    phMwIf_sHandle_t *mwIfHdl = &g_mwIfHandle;
    phMwIf_sQueueData_t *psQueueData = NULL;
    ALOGD("MwIf>%s:Enter",__FUNCTION__);
