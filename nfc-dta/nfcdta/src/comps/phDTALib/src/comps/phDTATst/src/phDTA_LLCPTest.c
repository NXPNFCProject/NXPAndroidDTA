                                   ...
                                   phMwIf_uLlcpEvtInfo_t*   puLlcpEvtInfo)
{
    ...
    if(eLlcpEvtType == PHMWIF_LLCP_SERVER_CONN_REQ_EVT)
    {
        ...
        psDtaLibHdl->pvCORemoteClientConnHandle = (void *)(((char*)0)+(puLlcpEvtInfo->sConnReq.wConnHandle));
        ...
        if (dwMwIfStatus != MWIFSTATUS_SUCCESS)
        {
            ...
            dwMwIfStatus = phMwIf_LlcpDisconnectConn(psDtaLibHdl->mwIfHdl,
                                                    (void *)(((char*)0)+(puLlcpEvtInfo->sConnReq.wConnHandle)));
            if (dwMwIfStatus != MWIFSTATUS_SUCCESS)
            {
    ...
    if(eLlcpEvtType == PHMWIF_LLCP_SERVER_CONN_ORIENTED_DATA_EVT)
    {
        ...
        phDtaLibi_PrintBuffer(sLlcpConnOrientedData.pbBuff,sLlcpConnOrientedData.dwBuffLength,(const uint8_t*)"DTALib>LLCP:Received Data");
        ...
        else
        {
            ...
            {
                dwMwIfStatus = phMwIf_LlcpSendData(psDtaLibHdl->mwIfHdl,
                                               psDtaLibHdl->pvCORemoteServerConnHandle,
                                               &sLlcpConnOrientedData);
...
DTASTATUS phDtaLibi_LlcpHandleUncongestedEvent()
{
    ...
    while(psDtaLibHdl->bIsLlcpCoRemoteServerLinkCongested == FALSE)
    {
            ...
            phDtaLibi_PrintBuffer(psLlcpConnOrientedData->pbBuff,psLlcpConnOrientedData->dwBuffLength,(const uint8_t*)"DTALib>LLCP:Pulled Data from queue");
            dwMwIfStatus = phMwIf_LlcpSendData(psDtaLibHdl->mwIfHdl,
                                           psDtaLibHdl->pvCORemoteServerConnHandle,
